...
public class StatisticsScreenForTesting extends GameScreen {
...
//    private SharedPreferences.Editor mPrefEditor = mGetPreferences.edit();
     ...
     */
    public StatisticsScreenForTesting(Game game) {
    ...
    public void setTotalWins(double winInput) {
        totalWins = winInput;
    }

    //Getters and setters for win/loss streak
    public int getGameStreak() { return gameStreak; }
    public void setGameStreak(boolean gameStreak) { //true input means a win, false means a loss.
        if(gameStreak) { //If its true...
            if(StatisticsScreenForTesting.gameStreak>=0) {
                StatisticsScreenForTesting.gameStreak++; //And the previous count is on a win (positive) streak then increment it.
            } else if(StatisticsScreenForTesting.gameStreak<0) {
                StatisticsScreenForTesting.gameStreak = 1; //And the previous count was on a loss (negative) streak then make the new 1 win into a win streak.
            }
        } else if (!gameStreak) { //else if its false...
            if (StatisticsScreenForTesting.gameStreak <= 0) {
                StatisticsScreenForTesting.gameStreak--; //And the previous count is on a loss (negative) streak then decrement it.
            } else if (StatisticsScreenForTesting.gameStreak > 0) {
                StatisticsScreenForTesting.gameStreak = -1; //And the previous count was on a win (positive) streak then make the new 1 loss into a loss streak.
            }
        }
    }
    ...
    public void setCardsDrawn(int cardsDrawn) { StatisticsScreenForTesting.cardsDrawn = cardsDrawn; }
    //Getters and setters for amount of cards player has played
    public int getCardsPlayed() { return cardsPlayed; }
    public void setCardsPlayed(int cardsPlayed) { StatisticsScreenForTesting.cardsPlayed = cardsPlayed; }

    //Setter for most recent time
    public double getRecentGameTime() { return recentGameTime; }
    public  void setRecentGameTime(double recentTime) {recentGameTime = recentTime; }
    //Getter and setter for total game time
    public  double getTotalGameTime() { return totalGameTime; }
    public  void setTotalGameTime(double totalTime) { totalGameTime = totalTime; }

    //Getters and setters for minions killed
    public  int getOwnMinionsKilled() { return ownMinionsKilled; }
    public  void setOwnMinionsKilled(int ownMinionsKilled) { StatisticsScreenForTesting.ownMinionsKilled = ownMinionsKilled; }
    public  int getEnemyMinionsKilled() { return enemyMinionsKilled; }
    public  void setEnemyMinionsKilled(int enemyMinionsKilled) { StatisticsScreenForTesting.enemyMinionsKilled = enemyMinionsKilled; }

    //Getters and setters for total losses
    public  double getTotalLosses() {
        return totalLosses;
    }
    public  void setTotalLosses(double lossInput) {
        totalLosses = lossInput;
    }

    //Getters and setters for most recent results
    public  String getMostRecentResult() {
        return mostRecentResult;
    }
    public  void setMostRecentResult(String resultInput) {
